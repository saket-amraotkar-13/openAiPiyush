...
interface MyContext {
  userId: string
  userName: string,
  fetchUserFromDB: (userId: string) => Promise<string>;
...
const getUserInfo = tool({
  ...
  parameters: z.object({}),
  execute: async (_input, ctx?: RunContext<MyContext>): Promise<string | undefined> => {
    const result = await ctx?.context.fetchUserFromDB(ctx?.context.userId);
    return result
  }
})
...
main('what is my name', {
  userId: '1',
  userName: 'Saket',
  fetchUserFromDB: async (id: string) => `UserId=1, UserName=Saket`
})